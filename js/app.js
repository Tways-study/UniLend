...
//   Abstraction    : SkeletonLoader, Toast — hide complex DOM logic
//                    behind clean static interfaces.
//   Encapsulation  : PasswordStrengthMeter — bundles strength data
//                    + logic together; Dashboard subclasses keep
//                    their state (channels, equipment cache) private.
//   Inheritance    : StudentDashboard & AdminDashboard extend Dashboard,
//                    reusing constructor, db reference, and _setupLogout().
//   Polymorphism   : Both subclasses override load() — the same call
//                    produces role-specific behaviour at runtime.
// ============================================================
...
// PRINCIPLE: Abstraction — hides repetitive HTML template
//            generation behind a clean static interface.
//            Callers never need to know the markup details.
// ============================================================
class SkeletonLoader {
  static rows(cols, count = 4) {
...
// ============================================================
// CLASS: Toast
// PRINCIPLE: Encapsulation — the iconMap data and all DOM
//            construction logic are bundled inside the class
//            and hidden from callers.
//            Abstraction   — callers only invoke Toast.show();
//            the implementation is invisible.
//            Polymorphism  — the same method call produces a
//            visually distinct result for each `type` value.
// ============================================================
class Toast {
  static #iconMap = {
    success: "bi-check-lg",
  ...
  static show(message, type = "info") {
    ...
    iconEl.innerHTML = `<i class="bi ${Toast.#iconMap[type] || Toast.#iconMap.info}"></i>`;
...
// PRINCIPLE: Encapsulation — the strength config (labels +
//            colours), the scoring algorithm (#evaluate), and
//            the DOM update (#update) are all private members.
//            Outside code cannot access or break the internals;
//            it simply instantiates the class and the meter
...
class PasswordStrengthMeter {
  ...
  constructor(passwordInputId, segIds, labelId, wrapId) {
    this.input = document.getElementById(passwordInputId);
    this.segs  = segIds.map(id => document.getElementById(id));
    this.label = document.getElementById(labelId);
    this.wrap  = document.getElementById(wrapId);
    if (this.input) this.input.addEventListener("input", () => this.#update());
  }
  ...
  #update() {
    ...
    const level = pw.length === 0 ? 0 : Math.max(1, this.#evaluate(pw));
    if (this.wrap) this.wrap.style.display = pw.length ? "block" : "none";
    this.segs.forEach((seg, i) => {
      if (seg) seg.style.background = i < level ? this.#config[level].color : "#e5e7eb";
    });
    if (this.label) {
      this.label.textContent = this.#config[level].label;
      this.label.style.color = this.#config[level].color;
    }
  }
}

// ============================================================
// CLASS: Dashboard  (Abstract Base Class)
// PRINCIPLE: Abstraction  — defines a shared interface (load,
//            _setupLogout) without providing a complete
//            implementation. Subclasses fill in the details.
//            Inheritance  — StudentDashboard and AdminDashboard
//            both extend this class, inheriting db, user, and
//            the _setupLogout() method.
// ============================================================
class Dashboard {
  ...
  // Polymorphic entry point — MUST be overridden by every subclass.
  async load() {
    throw new Error(`${this.constructor.name} must implement load()`);
  }

  // Shared logout logic inherited by both subclasses.
  _setupLogout(channels = []) {
    document.getElementById("logoutBtn")?.addEventListener("click", async () => {
      channels.forEach(ch => ch && this.db.removeChannel(ch));
...
// PRINCIPLE: Inheritance  — inherits db, user, _setupLogout().
//            Encapsulation — private state (#allEquipment,
//            #filterAvail, channels) is hidden; external code
//            cannot corrupt it.
//            Polymorphism  — overrides load() with the
//            student-specific initialisation sequence.
// ============================================================
class StudentDashboard extends Dashboard {
  #allEquipment     = [];
  #filterAvail      = "all";
  ...
  constructor(supabaseClient, user) {
    super(supabaseClient, user);
  }

  // Polymorphism: student-specific override of Dashboard.load()
  async load() {
    this.#displayGreeting();
    await this.#loadEquipment();
    await this.#loadMyRequests();
    this.#setupSearch();
    this.#setupReserveForm();
  ...
  #displayGreeting() {
    ...
    if (greeting) {
      ...
      greeting.appendChild(document.createTextNode(this.user.email));
    }
  }

  async #loadEquipment() {
    const grid = document.getElementById("equipmentGrid");
    if (grid && !grid.querySelector(".card")) grid.innerHTML = SkeletonLoader.cards(3);
    const { data, error } = await this.db.from("equipment").select("*").order("name");
    if (!error && data) {
      this.#allEquipment = data;
      this.#renderEquipmentCards(data);
    }
  }
  ...
  #setupSearch() {
    ...
    const applyFilters = () => {
      ...
      if (this.#filterAvail === "available")   filtered = filtered.filter(item => item.available > 0);
      if (this.#filterAvail === "unavailable") filtered = filtered.filter(item => item.available === 0);
      this.#renderEquipmentCards(filtered);
    };
    searchInput?.addEventListener("input", applyFilters);
    filterSelect?.addEventListener("change", () => {
      this.#filterAvail = filterSelect.value;
      applyFilters();
    });
  }
  ...
  async #loadMyRequests() {
    ...
    if (tbody && !tbody.querySelector("td[data-loaded]")) tbody.innerHTML = SkeletonLoader.rows(3);
  ...
  #setupReserveForm() {
    document.getElementById("confirmReserveBtn")?.addEventListener("click", async () => {
      ...
      if (!dateVal)                   { Toast.show("Please select a reservation date.", "error"); return; }
      if (!pickupVal || !returnVal)   { Toast.show("Please select pickup and return times.", "error"); return; }
      if (pickupVal >= returnVal)     { Toast.show("Return time must be after pickup time.", "error"); return; }
      if (!qtyVal || qtyVal < 1)     { Toast.show("Please enter a valid quantity.", "error"); return; }
      if (qtyMax && qtyVal > qtyMax) { Toast.show(`Only ${qtyMax} unit(s) available.`, "error"); return; }
      ...
      if (error) {
        Toast.show("Failed to submit reservation. Please try again.", "error");
      } else {
  ...
  #subscribeToRealtime() {
    this.#equipmentChannel = this.db.channel("equipment-changes")
      .on("postgres_changes", { event: "*", schema: "public", table: "equipment" }, () => this.#loadEquipment())
      .subscribe();
    this.#requestsChannel = this.db.channel("my-requests")
      .on("postgres_changes", { event: "*", schema: "public", table: "reservations" }, () => this.#loadMyRequests())
      .subscribe();
...
// PRINCIPLE: Inheritance  — inherits db, user, _setupLogout().
//            Encapsulation — private channels and all helper
//            methods (#loadInventory, #showConfirmModal, etc.)
//            are hidden inside the class.
//            Polymorphism  — overrides load() with the
//            admin-specific initialisation sequence.
// ============================================================
class AdminDashboard extends Dashboard {
  ...
  // Polymorphism: admin-specific override of Dashboard.load()
  async load() {
    await this.#loadInventory();
    await this.#loadPendingRequests();
    await this.#loadOverdueReservations();
    this.#setupRestockForm();
    this.#subscribeToRealtime();
  }

  // Encapsulated confirm dialog — admin-only concern, hidden from outside.
  #showConfirmModal({ title, body, okLabel, okColor, onConfirm }) {
  ...
  #updateStats(equipment, pendingCount) {
  ...
  async #loadInventory() {
    ...
    if (tbody && !tbody.querySelector("td[data-loaded]")) tbody.innerHTML = SkeletonLoader.rows(5);
    const { data, error } = await this.db.from("equipment").select("*").order("name");
  ...
  async #loadPendingRequests() {
    ...
    if (tbody && !tbody.querySelector("td[data-loaded]")) tbody.innerHTML = SkeletonLoader.rows(4);
    const { data, error } = await this.db
      .from("reservations")
      .select("*, equipment(name)")
      .eq("status", "pending")
    ...
    this.#updateStats(null, data?.length ?? 0);
      ...
      btn.addEventListener("click", () => {
        ...
        this.#showConfirmModal({
          ...
          onConfirm: async () => {
            ...
            if (error) {
              Toast.show("Failed to deny request.", "error");
              btn.disabled  = false;
  ...
  async #loadOverdueReservations() {
    ...
    this.#updateOverdueStat(overdueData.length);
      ...
      btn.addEventListener("click", () => {
        ...
        this.#showConfirmModal({
          ...
          onConfirm: async () => {
            ...
            if (rpcErr) {
              Toast.show("Failed to mark as returned.", "error");
              btn.disabled  = false;
              btn.innerHTML = `<i class="bi bi-box-arrow-in-left"></i> Mark Returned`;
            } else {
              Toast.show("Equipment marked as returned.", "success");
            }
          },
        });
      });
  ...
  #setupRestockForm() {
    ...
    document.getElementById("confirmRestockBtn")?.addEventListener("click", async () => {
      ...
      if (!amount || amount < 1) { Toast.show("Enter a valid number of units to add.", "error"); return; }
      if (amount > 999)          { Toast.show("Restock amount cannot exceed 999 units at once.", "error"); return; }

      btn.disabled  = true;
      btn.innerHTML = `<span class="spinner-border spinner-border-sm me-1"></span>Restocking\u2026`;

      const { error } = await this.db.rpc("restock_equipment", { eq_id: itemId, amount });

      if (error) {
        Toast.show("Failed to restock. Please try again.", "error");
      } else {
        bootstrap.Modal.getInstance(document.getElementById("restockModal")).hide();
        Toast.show(`Restocked successfully \u2014 ${amount} unit${amount !== 1 ? "s" : ""} added.`, "success");
      }
      btn.disabled  = false;
  ...
  #subscribeToRealtime() {
    this.#inventoryChannel = this.db.channel("admin-equipment")
      .on("postgres_changes", { event: "*", schema: "public", table: "equipment" }, () => this.#loadInventory())
      .subscribe();
    this.#pendingChannel = this.db.channel("admin-reservations")
...
if (currentPage === "login") {
  ...
  // Encapsulation: PasswordStrengthMeter bundles the config data,
  // scoring logic, and DOM wiring into one object. The login page
  // only needs to instantiate it — no implementation details exposed.
  new PasswordStrengthMeter(
    "regPassword",
    ["pwSeg1", "pwSeg2", "pwSeg3", "pwSeg4"],
    "pwStrengthLabel",
    "pwStrengthWrap"
  );
...
// ============================================================
// STUDENT DASHBOARD — Inheritance + Polymorphism
// StudentDashboard extends Dashboard and overrides load().
// ============================================================
if (currentPage === "student") {
  ...
  if (session) {
  // Polymorphism: load() dispatches the student-specific sequence
  // even though the call site only knows about the Dashboard interface.
  const dashboard = new StudentDashboard(supabase, session.user);
  await dashboard.load();

  // (All student logic is now encapsulated in StudentDashboard above.)

  } // end: if (session) guard
}

// ============================================================
// ADMIN DASHBOARD — Inheritance + Polymorphism
// AdminDashboard extends Dashboard and overrides load().
// ============================================================
if (currentPage === "admin") {
  ...
  if (profile?.role === "admin") {
  // Polymorphism: load() dispatches the admin-specific sequence
  // even though the call site only knows about the Dashboard interface.
  const dashboard = new AdminDashboard(supabase, session.user);
  await dashboard.load();

  // (All admin logic is now encapsulated in AdminDashboard above.)
  // ---- placeholder to keep guard structure intact ----
  function _adminLoaded() {

  } // _adminLoaded

  // ---- (legacy block removed — logic lives in AdminDashboard) ----
  function _noop() {
